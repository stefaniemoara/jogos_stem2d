...
snake = [(200, 200), (210, 200), (220, 200)] #representar a cobra como uma lista de segmentos, uma dupla de [(x,y)] posicionada dentro do nosso quadrado da tela
...
apple_pos = (10*random.randint(0,59), 10*random.randint(0,59)) #dupla de números aleatorios entre 0 e 590, que é o ultimo lugar que posso colocar antes de sair da tela
...
while True:
    ...
    if my_direction == UP:
        snake[0] = (snake[0][0], snake[0][1] - 10) # movimentando a cabeça da cobra snake[0] sendo que y diminui na tela para cima
    if my_direction == DOWN:
        snake[0] = (snake[0][0], snake[0][1] + 10) # movimentando a cabeça da cobra snake[0] sendo que y aumenta na tela para baixo
    ...
    for pos in snake: #para cada posição da snake, fazer atualização da tela para andar, com função blit
        #screen.blit(pygame.Surface((10,10)), pos) #passando um gráfico para a tela com um sprite e uma posição onde aparecer o sprite
        screen.blit(snake_skin, pos) #substitui o Surface. poderia utilizar imagem, qualquer coisa gráfica que irá se mover, chamamos sprite
